...
class TestWorker:
    ...
    def test_valid_message(self, mock_resource):
        status, result = worker.process_message(self.mock_docker, self.mock_image, self.mock_db, self.msg_id, '{"command":[]}', self.worker_id)
        assert(status == worker.STATUS_COMPLETE)
        assert(result == docker_success)

    @mock.patch('worker.launch_container', return_value = docker_success)
    def test_valid_message_s3_stdin(self, mock_resource):
        status, result = worker.process_message(self.mock_docker, self.mock_image, self.mock_db, self.msg_id, '{"stdin":"s3://lanlytics/path/to/input/test.geojson", "command":[]}', self.worker_id)
        assert(status == worker.STATUS_COMPLETE)
        assert(result == docker_success)

    @mock.patch('worker.launch_container', return_value = docker_success)
    def test_valid_message_s3_cmd(self, mock_resource):
        status, result = worker.process_message(self.mock_docker, self.mock_image, self.mock_db, self.msg_id, '{"command":[{"type":"output", "value":"s3://lanlytics/path/to/input/test.geojson"}]}', self.worker_id)
        assert(status == worker.STATUS_COMPLETE)
        assert(result == docker_success)
    ...
    def test_invalid_schema_message(self):
        status, result = worker.process_message(self.mock_docker, self.mock_image, self.mock_db, self.msg_id, '{"bloop":[]}', self.worker_id)
        assert(status == worker.STATUS_FAILED)
